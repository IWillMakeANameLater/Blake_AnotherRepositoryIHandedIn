...
public class Gridder extends javax.swing.JFrame 
    ...
    public boolean isAlive(int c, int r) { // Was this for some kind of Conway's game of life ???????
    ...
    public int countNeighbors(int c, int r){
       return(0);
    } // And I'd imagine this was somehow related to isAlive method
    ...
    public void step() {
        ...
        //leave this in here or you won't see anything happen!  || This message can't stop me because I can't read! ||
        daw();
    }
    ...
    private void jButtonRunActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonRunActionPerformed
        ...
        if (active==false) {
            if (timmy==null) {
              timmy=new Timer(stepDelay,this); // WHY IS It CALLED A TIMMY
              timmy.setInitialDelay(100); 
              timmy.setDelay(stepDelay);
            }
            
            timmy.start(); // this name physically hurts to read
            active=true;
            jButtonRun.setText("Pause...");
        }
        else { //turn timer off
            active=false;
            timmy.stop(); // no, don't stop timmy, stop yourself from naming it as timmy
            jButtonRun.setText("Run...");
        }
    ...
    private void jSliderDelayStateChanged(javax.swing.event.ChangeEvent evt) {//GEN-FIRST:event_jSliderDelayStateChanged
        ...
        if (timmy!=null) // WHO IS TIMMY?????????????
            timmy.setDelay(stepDelay);
    }//GEN-LAST:event_jSliderDelayStateChanged

    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed //Currently can become stuck in an infinite loop
        int steps = 100;
        while(steps > 0){
            int randX = (int) (Math.random() * gridCount);
            int randY = (int) (Math.random() * gridCount);
            if(grid[randX][randY] == 0){ // Check to ensure that the square isn't already white
                grid[randX][randY] = 1;
                steps--;
            }
        }
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        int randCol = (int) (Math.random() * gridCount);
    ...
    private void jButton4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton11ActionPerformed
        ...
        textInfo.setText(Integer.toString(total) + " white squares on the grid.");
    ...
    private void jButton6ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton11ActionPerformed
        for(int i = 0; i<gridCount; i++){
            for(int j = 0; j<gridCount; j++){
                int newValue = 1-grid[i][j]; // So it turned out you can't convert an int to a boolean because there is no garuntee that the int would only be 0 or 1 and is normally too big to be converted into boolean. However 'Reverened Gonzo' gave a really simple solution that achieved what I wanted to do (credit: https://stackoverflow.com/a/4084058)
                grid[i][j] = newValue;
            }
        }
        draw();
    ...
    private void jButton7ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton11ActionPerformed
        ArrayList<Integer> markedX = new ArrayList<>();
        ArrayList<Integer> markedY = new ArrayList<>();

        for(int i = 0; i < gridCount; i++){
            for(int j = 0; j < gridCount; j++){
                if(grid[i][j] == 0){ // only check blacks
        ...
        for(int l = 0; l < markedX.size(); l++){
            grid[markedX.get(l)][markedY.get(l)] = 1;
        }
        draw();
    }//GEN-LAST:event_jButton11ActionPerformed

    private boolean checkSurroundings(int posX, int posY){
        int surroundingWhites = 0;
        for(int x = posX-1; x < posX+2; x++){
            for(int y = posY-1; y < posY+2; y++){
                if(safeCheck(x, y) == 1){
                    surroundingWhites++;
                }
                if(surroundingWhites >= 2){
                    return true;
                }
            }
        }
    ...
    private int safeCheck(int posX, int posY){
        if(posX > gridCount-1 || posX < 0){
            return -1;
        } else if (posY > gridCount-1 || posY < 0){
            return -1;
        }
    ...
    private void jButton8ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton11ActionPerformed
        int[][] shiftedGrid = new int[gridCount][gridCount];

        for(int i = 0; i < gridCount; i++){
            for(int j = 0; j < gridCount; j++){
                shiftedGrid[Math.floorMod(i-1, gridCount)][j] = grid[i][j];
            }
        }
    ...
    private void jButton9ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton11ActionPerformed
        // row becomes col & col becomes maxSize-row (and add -1)
        int[][] rotatedGrid = new int[gridCount][gridCount];

        for(int i = 0; i < gridCount; i++){
            for(int j = 0; j < gridCount; j++){
                rotatedGrid[j][gridCount-1-i] = grid[i][j];
            }
        }
